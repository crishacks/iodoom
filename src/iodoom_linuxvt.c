// iodoom_linuxvt.c – Linux Virtual Terminal platform implementation
// Copyright (C) 2025 Techflash
// Based on doomgeneric_sdl.c (original) – rebranded for iodoom
...
void DG_Init() {
	...
	fbOffsetX = ((fbWidth - IODOOM_RESX) / 2) * fbBytesPerPixel;
	fbOffsetY = ((fbHeight - IODOOM_RESY) / 2) * fbStride;
...
void DG_DrawFrame() {
	...
	// larger than the iodoom render resolution.
	for (int line = 0; line < IODOOM_RESY; line++) {
		memcpy(
			(void *)((uintptr_t)(fbPtr) + (fbStride * line) + fbOffsetY + fbOffsetX),
				(void *)(((uintptr_t)DG_ScreenBuffer) + (IODOOM_RESX * line * fbBytesPerPixel)),
			 (fbBytesPerPixel * IODOOM_RESX)
		);
	}
